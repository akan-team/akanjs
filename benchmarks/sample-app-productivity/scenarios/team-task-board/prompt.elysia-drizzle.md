# Elysia + Drizzle + React/Vite Stack Appendix
...
- Put Elysia API/server code in a clear server module, for example `src/server.ts` or `src/api.ts`.
- Use Drizzle with SQLite as the persistence layer, with schema definitions kept separate from React components.
- Make `bun run dev --host 127.0.0.1` start both the Vite frontend and the Elysia backend needed by the smoke test.
- Do not replace Elysia with another server framework or Prisma.
